  ...
  return (
    <div className="space-y-6">
      ...
      {progressSummary?.hasComparison && (
        <div className="p-4 sm:p-5 rounded-2xl bg-neutral-900/80 border border-red-500/30 space-y-4">
            ...
            {progressSummary.deltaMuscle !== null && (
              <div className="p-3 rounded-xl bg-white/[0.03] border border-white/5">
                ...
                <p className="text-base font-bold font-mono text-emerald-400 flex items-center gap-1">
                  ...
                  {progressSummary.deltaMuscle > 0 ? `+${progressSummary.deltaMuscle}%` : `${progressSummary.deltaMuscle}%`}
                </p>
              </div>
            )}
            ...
            {progressSummary.deltaFat !== null && (
              <div className="p-3 rounded-xl bg-white/[0.03] border border-white/5">
                ...
                <p className="text-base font-bold font-mono text-amber-400 flex items-center gap-1">
                  ...
                  {progressSummary.deltaFat > 0 ? `+${progressSummary.deltaFat}%` : `${progressSummary.deltaFat}%`}
                </p>
              </div>
            )}
            ...
            {progressSummary.deltaWaist !== null && (
              <div className="p-3 rounded-xl bg-white/[0.03] border border-white/5">
                ...
                <p className="text-base font-bold font-mono text-white flex items-center gap-1">
                  ...
                  {progressSummary.deltaWaist > 0 ? `+${progressSummary.deltaWaist} cm` : `${progressSummary.deltaWaist} cm`}
                </p>
              </div>
            )}
            ...
            {progressSummary.deltaWeight !== null && (
              <div className="p-3 rounded-xl bg-white/[0.03] border border-white/5">
                ...
                <p className="text-base font-bold font-mono text-white flex items-center gap-1">
                  ...
                  {progressSummary.deltaWeight > 0 ? `+${progressSummary.deltaWeight} kg` : `${progressSummary.deltaWeight} kg`}
                </p>
              </div>
            )}
          </div>
      ...
      {loading ? (
        ...
        <div className="p-10 rounded-2xl border border-dashed border-neutral-200 dark:border-white/10 text-center bg-neutral-50 dark:bg-white/[0.02]">
          <Ruler className="w-10 h-10 text-neutral-400 mx-auto mb-3" />
          <h4 className="text-sm font-condensed font-bold uppercase tracking-wider text-white">
            Sin valoraciones registradas
          </h4>
          <p className="text-xs text-neutral-400 mt-1 max-w-sm mx-auto">
            Registra la primera valoración antropométrica para comenzar a graficar y medir los perímetros y avances del alumno.
          </p>
          {isTrainer && (
            <Button
              size="sm"
              ...
              className="mt-4 font-condensed uppercase font-bold text-xs"
            >
              ...
              <tbody className="divide-y divide-neutral-100 dark:divide-white/5 font-mono text-xs whitespace-nowrap">
                {assessments.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-3 font-bold text-white">{item.weight ? `${item.weight} kg` : '--'}</td>
                    <td className="p-3 text-amber-400 font-bold">{item.fatPercentage ? `${item.fatPercentage}%` : '--'}</td>
                    <td className="p-3 text-blue-400">{item.waterPercentage ? `${item.waterPercentage}%` : '--'}</td>
                    <td className="p-3 text-emerald-400 font-bold">{item.musclePercentage ? `${item.musclePercentage}%` : '--'}</td>
                    <td className="p-3 text-purple-400">{item.bonePercentage ? `${item.bonePercentage}%` : '--'}</td>
                    {isTrainer && (
